...
export function Rankings({
  ...
  const max = rubricList.reduce((sum, r) => sum + r.max, 0);
  ...
  const rows = useMemo(() => {
    ...
    return teams
      .map((team) => {
        ...
        const total = mine.reduce((sum, s) => sum + s.total, 0);
        const scored = mine.some((s) => Object.keys(s.values ?? {}).length > 0);
        return { team, total, scored, perRubric: mine };
      })
      .sort(
        (a, b) =>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <span className="text-sm text-zinc-400">
          {scoredCount} of {rows.length} teams scored · {max} points available
        </span>
        <BandLegend />
      </div>
          ...
          <tbody className="divide-y divide-white/5">
            {rows.map((row, i) => (
              ...
              >
                ...
                <td className="px-3 py-2.5 text-right text-lg font-bold tabular-nums">
                  {row.scored ? row.total : "—"}
                </td>
                <td className="px-3 py-2.5">
                  <BandChip total={row.total} max={max} scored={row.scored} />
                </td>
              </tr>
            ))}
          </tbody>
